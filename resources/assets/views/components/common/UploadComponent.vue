...
<div>
  <blockquote>
    <p class="col-md-3 col-xs-12"><a href="{{ upload_url }}{{ example_file_url }}?access_token={{access_token}}">Download Example File <strong><i class="fa fa-download"></i></strong></a></p>
  ...
  <div class="panel panel-default" v-show="show_upload_notes">
...
<script>
  import {ACCESS_TOKEN} from '../../../js/vue.config.js'
  export default {
    props: [
      'upload_url',
      'show_upload_notes',
    ...
    methods: {
      initUpload() {
        $("input[name='access_token']").val(this.access_token);
        $('#fileupload').fileupload({
          url: this.upload_url+'?access_token='+this.access_token
        });
        // Load existing files:
